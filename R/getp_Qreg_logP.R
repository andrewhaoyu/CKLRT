...
getp_Qreg_logP = function(null, LR, qmax = 1){
  ...
  null2= null[null > 0]
  null2= null2[null2 >= quantile(null2, 1-qmax)]
  f    = function(x){    # objective function wrt scale, dof
    scl   = x[1]; dof = x[2]
    the_p = 1 - pchisq(null2/scl, df = dof)
    emp_p = 1 - (rank(null2)-0.5)/length(null2) # bigger null2, smaller p
    rr    = sum(log(the_p) -log(emp_p))^2
    return(rr)
  }
  test= optim(c(0.5, 0.1) , f)
  scl = test$par[1]
  dof = test$par[2]
  final_p =(1-prop)*pchisq(scl*LR, df = dof,  lower.tail = F)
...
getp_Qreg_dir = function(null, LR, qmax = 1){
  ...
  f    = function(x){    # objective function wrt scale, dof
    ...
    the_q = scl *qchisq(p = emp_p , df = dof)
